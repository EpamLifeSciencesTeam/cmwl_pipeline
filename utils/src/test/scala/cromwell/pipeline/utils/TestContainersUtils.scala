...
object TestContainersUtils {
  ...
  def getConfigForPgContainer(container: PostgreSQLContainer): Config =
    config("database.postgres_dc.db.properties.portNumber" ->
  ...
  def getMongoContainer(mongoImageName: String = "mongo"): GenericContainer[_] = {
    new GenericContainer[GenericContainer](mongoImageName)
